...
| AI | 役割 |
|---|---|
| Claude Code | 実行（コード・ファイル作成・変更） |
| AI Meeting Hub | 正本（決定・議題・次アクションの唯一の記録場所） |
| ChatGPT / 外部AI | 外部レビュー（矛盾チェック・リスク指摘・判断確認） |

## ChatGPTへの持ち込みルール

ChatGPTは外部レビュー役として使う。  
ChatGPTの回答はそのまま正本にせず、必要な内容だけ `decision-log.md` / `next-actions.md` / `meeting-log.md` に反映する。

**ChatGPTに持ち込むときの前文テンプレート：**

```
これはAI会議ハブの内容確認です。
ChatGPTは外部レビューとして、矛盾・危険・次アクションだけ確認してください。
最終判断はAI会議ハブに戻します。
```
...
**ChatGPTがやってはいけないこと：**
- 決定の上書き（正本は `decision-log.md`）
- `decision-log.md` に記録されていない新決定の実行指示
- AI Meeting Hub の議題・決定・ログの直接編集
