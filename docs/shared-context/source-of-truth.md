...
### 運用設計の基本原則
- **管理主体**: Claude（プロジェクト管理・共通台帳更新・実行方針の決定）
- **実行担当**: Claude Code（ファイル作成・GitHub操作・PR・ローカル作業）
- **外部参謀**: ChatGPT（外部レビュー・リスク判定・選択肢整理・Claudeへの指示文作成）
- **台帳保管**: GitHub / docs（共通台帳の保存場所）

ChatGPTは管理主体でも実行担当でもない。判断補助・監査・Claude Codeへの指示文作成が役割。

### 各担当の役割（確定・運用中）

| 担当 | 役割 | 実行権限 |
|-----|------|---------|
| **Claude** | プロジェクト管理・共通台帳更新・構造化・実行方針決定 | ◎ 管理主体 |
| **Claude Code** | ファイル作成・GitHub操作・PR・ローカル作業 | ◎ PC実行担当 |
| **ChatGPT** | 外部レビュー・リスク判定・選択肢整理・Claudeへの指示文作成 | ✗ 実行しない |
| **GitHub / docs** | 共通台帳の保存・ソースコード管理・CI/CD | — 保管場所 |
| **Netlify** | フロントエンドホスティング・Serverless Functions実行 | — インフラ |

### 各ツールの役割（確定・プロダクト用途）

| ツール | 役割 |
|-------|------|
| **Claude API** | IRテキスト分析・市場レポート生成・web検索（本番機能） |
| **GitHub Actions** | 朝・夕レポート自動配信 |
